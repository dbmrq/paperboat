...
​
 Summary
The Agent CLI can be configured using a JSON file, with global settings in ~/.cursor/cli-config.json or $env:USERPROFILE\.cursor\cli-config.json and project-specific settings in <project>/.cursor/cli.json. Required fields include version, editor settings, and permissions, while optional fields allow for model selection, HTTP/1.1 fallback, and attribution settings.
​
