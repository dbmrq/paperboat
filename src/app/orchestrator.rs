...
use crate::mcp_server::{AgentSpec, ToolCall, ToolResponse};
...
impl App {
    ...
    async fn create_orchestrator_session_with_retry(
        ...
        Err(last_error
            ...
            .context(format!(
                "All models in fallback chain failed for orchestrator: {model_chain:?}"
            )))
    }
    ...
    /// Main orchestrator event loop implementation.
    ///
    /// This is a large method (~440 lines) because it implements the core async event loop
    /// that handles:
    /// - Tool call processing via MCP (`decompose`, `spawn_agents`, `complete`, etc.)
    /// - ACP message handling for session updates
    /// - Configuration updates from TUI
    ///
    /// The method uses `tokio::select!` to multiplex between these event sources, and the
    /// tool call handling requires inline match arms to properly manage the async state.
    /// Breaking this into smaller functions would require passing mutable references through
    /// complex async boundaries, which would make the code harder to follow.
    ///
    /// Tool call handlers that don't require access to `tool_rx` have been extracted to
    /// `orchestrator_handlers.rs` to reduce this file's size.
